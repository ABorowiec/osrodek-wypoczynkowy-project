	...
	if ($level == 6 || $level == 5 || $level == 3 || $level == 1 ) // sprawdź uprawnienia
	{
		if (isset($_POST['wyswietl'])) // jeśli kliknięto przycisk wyświetl
		{
			...
			if (isset($_POST['lista'])) // jeśli wybrano promocje
			{
				...
				$wybrany_rekord = mysql_query("SELECT * FROM promocja WHERE Nazwa = '$edytowane'"); // wybierz rekord do wyświetlenia
		...
		elseif (isset($_POST['wyszukaj'])) //jeśli klinięto wyszukaj
		{
			...
			else // ...a jeśli zostało
			{
				...
				mysql_select_db($db_name); //wybierz bazę
					// Else
					$wyszukane = mysql_query("SELECT * FROM promocja WHERE Nazwa LIKE '%$nazwa_szukanego_rekordu%' ORDER BY Nazwa ASC");
		...
		elseif ((!isset($_POST['dodaj'])) and (!isset($_POST['usun'])) and (!isset($_POST['edytuj'])))
		{
			...
			$data = "SELECT * FROM promocja ORDER BY Nazwa ASC LIMIT $na_stronie OFFSET $offset";
		...
		if (isset($_POST['edytuj'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				$connection=mysql_connect ($db_host, $db_user, $db_pass) or die ("Próba połączenie z bazą danych nie powiodła się. Spróbuj ponownie");
				mysql_select_db($db_name);
				$edytowany_rekord = mysql_query("SELECT * FROM promocja WHERE Nazwa = '$edytowane'");
				...
				if ($row = mysql_fetch_row($edytowany_rekord))
				{
					...
					echo "Znizka: <input type='text' name='znizka' value='". $row[3] ."' size='15' /> <br/><br/>";
					echo "Opis: <input type='text' name='opis' value='". $row[4] ."' size='15' /> <br/><br/>";
					echo"<input type='hidden' name='stara_nazwa' value ='".$row[1]."' />";
					echo"<input type='hidden' name='stary_domek' value ='".$row[2]."' />";
					echo"<input type='hidden' name='stara_znizka' value ='".$row[3]."' />";
					echo"<input type='hidden' name='stary_opis' value ='".$row[4]."' />";
