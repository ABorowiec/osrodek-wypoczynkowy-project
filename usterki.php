...
if(isset($_SESSION['zalogowany'])) // jeśli zalogowany
{
	...
	if ($level == 6 || $level == 5 || $level == 4 ) // sprawdź uprawnienia
	{
		if (isset($_POST['wyswietl'])) // jeśli kliknięto przycisk wyświetl
		{
			...
			if (isset($_POST['lista'])) // jeśli wybrano usterke
			{
				//echo $_POST['lista'];
				...
				$wybrany_rekord = mysql_query("SELECT * FROM usterka WHERE Nazwa = '$edytowane'"); // wybierz rekord do wyświetlenia
				$liczba_wierszy = @mysql_num_rows($wybrany_rekord); // przypisz zmiennej
				echo "<h1>Wybrana usterka</h1>";
				if ($liczba_wierszy > 0) //jeśli istnieją jakieś rekordy
				{
					...
					echo"<br/><br/>";
					echo"<br/><br/>";
					echo "<a href='usterki.php'>Wróć</a>";
					$lista = $wybrany_rekord;
				}
			}
			else{ //...a jeśli nie
				echo "Wybierz usterke  "; //wyświetl komunikat
				echo"<br/><br/>";
				echo"<br/><br/>";
				echo "<a href='usterki.php'>Wróć</a>";
			}
		...
		elseif (isset($_POST['wyszukaj'])) //jeśli klinięto wyszukaj
		{
			...
			if  ((empty($nazwa_szukanego_rekordu))) //jeśli pole nie zostało wypełnione
			{
				...
				echo "<a href='usterki.php'>Wróć</a>";
				header( 'refresh: 5; url=usterki.php' );
				$pagenum = 1;
			}
			else // ...a jeśli zostało
			{
				...
				mysql_select_db($db_name); //wybierz bazę
				// Else
				$wyszukane = mysql_query("SELECT * FROM usterka WHERE Nazwa LIKE '%$nazwa_szukanego_rekordu%' ORDER BY Nazwa ASC");
				...
				echo "<br><br><a href='usterki.php'>Wróć</a>";
			}
		}
		elseif ((!isset($_POST['dodaj'])) and (!isset($_POST['usun'])) and (!isset($_POST['edytuj'])))
		{
			...
			$wszystkie = mysql_query("SELECT * FROM usterka ORDER BY Nazwa ASC");
			//pejdżowanie
			...
			$data = "SELECT usterka.ID, domek.Nazwa FROM usterka, domek WHERE usterka.ID_Domek = domek.ID ORDER BY usterka.ID ASC LIMIT $na_stronie OFFSET $offset";
		...
		If (!empty($lista)) //jeśli zmienna $lista nie jest pusta
		{
			...
			echo"<option selected disabled>Wybierz usterke</option>";
		...
		if (isset($_POST['dodaj'])) // jeśli klinieto 'dodaj'
		{
			...
			echo "<input type='submit' name='dodaj' value='Dodaj usterke' />";
			echo "</form>";
			echo "<a href='usterki.php'>Wróć</a>";
			//$lista = $wybrany_rekord;
		}
		if (isset($_POST['edytuj'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				$connection=mysql_connect ($db_host, $db_user, $db_pass) or die ("Próba połączenie z bazą danych nie powiodła się. Spróbuj ponownie");
				mysql_select_db($db_name);
				$edytowany_rekord = mysql_query("SELECT * FROM usterka WHERE Nazwa = '$edytowane'");
				echo "<h1>Edytuj usterke</h1>";
				if ($row = mysql_fetch_row($edytowany_rekord))
				{
					echo "<form method='post' action='zatwierdz.php'>";
					echo "Nazwa: <input type='text' name='nazwa' value='". $row[1] ."' size='15' /> <br/><br/>";
					echo "Domek: <input type='text' name='domek' value='". $row[2] ."' size='15' /> <br/><br/>";
					...
					echo "<input type='submit' name='edytuj' value='Edytuj usterke' />";
					echo "</form>";
					echo "<a href='usterki.php'>Wróć</a>";
				...
				echo "Wybierz usterke  ";
				echo "</br></br></br><a href='usterki.php'>Wróć</a>";
			}
		}
		if (isset($_POST['usun'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				$connection=mysql_connect ($db_host, $db_user, $db_pass) or die ("Próba połączenie z bazą danych nie powiodła się. Spróbuj ponownie");
				mysql_select_db($db_name);
				$usuwany_rekord = mysql_query("SELECT * FROM usterka WHERE Nazwa = '$usuwane'");
				echo "<h1>Usunąć usterke z bazy?</h1>";
				if ($row = mysql_fetch_row($usuwany_rekord)) // jeśli wybrano rekord
				{
					...
					echo "<input type='hidden' name='opis' value='". $row[3] ."' size='15' /> <br/><br/>";
					echo "<input type='submit' name='usun' value='Usuń usterke' />";
					echo "</form>";
					echo "<a href='usterki.php'>Wróć</a>";
