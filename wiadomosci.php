...
if(isset($_SESSION['zalogowany']))
{
	...
	if ($level == 2 or $level == 5 or $level == 6)
	{
		...
		elseif (isset($_POST['usun'])) // jeśli kliknięto przycisk usuń
		{
			...
			if (isset($_POST['lista'])) // jeśli wybrano domek
			{
			$id_domku =  $_POST['lista']; // przypisz zmienne
			$id_zamowienia =  $_POST['id'];
			$nazwa_domku =  $_POST['nazwa_domku'];
		...
		$zamowienia = mysql_query("SELECT * FROM zamowienia ORDER BY ID ASC");
			...
			if ($liczba_wierszy > 0)
			{
				...
				{
					echo "<input type=hidden name=id_domku value='". $row3[1] ."'>";  //id_domku
					echo "<input type=hidden name=nazwa_domku value='". $row3[1] ."'>";  //nazwa_domku poprawić nr rowa
