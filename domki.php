...
if(isset($_SESSION['zalogowany'])) // jeśli zalogowany
{
	...
	if ($level == 6 || $level == 5 || $level == 4 || $level == 2 ) // sprawdź uprawnienia
	{
		...
		if (isset($_POST['wyswietl'])) // jeśli kliknięto przycisk wyświetl
		{
			...
			if (isset($_POST['lista'])) // jeśli nie wybrano rekordu
			{
			...
			else{ //...a jeśli takie rekordy nie istnieją
				
				echo "Wybierz domek kurwiu"; //wyświetl komunikat
		...
		if (!isset($_POST['wyswietl']) and (!isset($_POST['dodaj'])) and (!isset($_POST['usun'])) and (!isset($_POST['edytuj']))) // jeśli nie wypełniono żadnego formularza
		{
